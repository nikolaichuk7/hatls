...
**Open question for the working groups.** `CHIP_ID` answers "same silicon?", not "same instance?".
Two guests on one socket share it; a migrated guest loses it; a shared-tenancy guest never had it.
Which claim should carry instance identity across SEV-SNP, TDX and Nitro is unresolved, and we
would rather it were settled in the open than guessed per implementation.
...
- The mandate itself is trusted and is not yet in the threat model: replication, audit, and what a
  relying party can still decide when the mandate is unavailable or compromised.
